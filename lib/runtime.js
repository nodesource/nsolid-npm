...
async function checkVersion (lts) {
  ...
  try {
    ...
    for (const key in availableVersions) {
      const currentVersion = availableVersions[key]?.artifacts?.[platform]?.[`nsolid-${lts}`] ||
        availableVersions[key]?.binaries?.[platform]?.[`nsolid-${lts}`] ||
        availableVersions[key]?.binaries?.[platform]?.[arch]?.[`nsolid-${lts}`]

      if (currentVersion) {
        version = key
        tarUrl = currentVersion
        break
      }
    }
