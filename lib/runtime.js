...
async function checkVersion (lts) {
  ...
  try {
    ...
    for (const key in availableVersions) {
      const currentVersion = availableVersions[key].artifacts || availableVersions[key].binaries
      if (currentVersion[artifactVersion] && currentVersion[artifactVersion][`nsolid-${lts}`]) {
        version = key
        tarUrl = currentVersion[artifactVersion][`nsolid-${lts}`]
        break
      }
    }
