...
async function checkVersion (lts) {
  const versionsPath = path.join(os.tmpdir(), 'metadata.json')
...
function extractRuntime (tarPath, runtimePath, runtimeDir) {
  ...
  output.debug(deleteCommand(path.join(os.tmpdir(), runtimeDir)))
  execSync(deleteCommand(path.join(os.tmpdir(), runtimeDir)))

  // Extract the tar
  output.debug(extractCommand(tarPath, os.tmpdir()))
  execSync(extractCommand(tarPath, os.tmpdir()))

  // Move the files to the final destination
  output.debug(moveCommand(path.join(os.tmpdir(), runtimeDir), runtimePath))
  execSync(moveCommand(path.join(os.tmpdir(), runtimeDir), runtimePath))

  // Clean the tmp path and the tar
  output.debug(deleteCommand(path.join(os.tmpdir(), runtimeDir)))
  execSync(deleteCommand(path.join(os.tmpdir(), runtimeDir)))
  output.debug(deleteCommand(tarPath))
  execSync(deleteCommand(tarPath))
}
